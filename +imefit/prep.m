...
% T = [1 ntrials] cell, T(1) = [2 1] double
    ...
    for ii = 1:ntrs
        ...
        tms = B.time(it);
        goaltms = firstTimeToKeep:max(tms);
        alltms = sort(tms(tms >= firstTimeToKeep));
        if ~isequal(alltms', goaltms) && ~didWarn
            nm = numel(goaltms) - numel(alltms);
            warning(['Not all times are present. Missing ' ...
                num2str(nm) ...
