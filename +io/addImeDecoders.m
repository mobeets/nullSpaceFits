function D = addImeDecoders(D)
    ...
    for ii = 1:numel(D.blocks)
        ...
        D.blocks(ii).thetaImeGrps = io.thetaGroup(ths_ime, ...
            io.thetaCenters(8));
        D.blocks(ii).thetaImeGrps16 = io.thetaGroup(ths_ime, ...
            io.thetaCenters(16));
        D.blocks(ii).thetaActualImeGrps = io.thetaGroup(thsact_ime, ...
            io.thetaCenters(8));
        D.blocks(ii).thetaActualImeGrps16 = io.thetaGroup(thsact_ime, ...
            io.thetaCenters(16));
    end
end
...
%     movVec = diff(pos_ime); % or do we compare true pos to next pos_ime?
    ...
    ths_ime = arrayfun(@(t) io.computeAngle(vec2trg(t,:), [1; 0]), ...
        1:size(vec2trg,1))';
    ths_ime = mod(ths_ime, 360);
    
    angErr_ime = arrayfun(@(t) io.computeAngle(movVec(t,:), ...
        vec2trg(t,:)), 1:size(vec2trg,1)-1);
    angErr_ime = [angErr_ime nan]'; % for last time step
    
    thsact_ime = arrayfun(@(t) io.computeAngle(movVec(t,:), [1; 0]), ...
        1:size(movVec,1))';
