function m = findBestNeuronMean(Z, NB, RB, gs, dec)
...
% note: the below is silly because we don't actually want to insist that
% the mean spiking activity for a bin be non-negative...
%     m - A2*(A*m - A*nu - r) = m - A2*A*m + A2*(A*nu + r) >= 0
%     (I - A2*A)m >= -A2*(A*nu + r)
%     Aeq = A2*A - eye(numel(nu));
%     beq = A2*(A*nu + Zr(1,:)'); % mean spike prediction for bin j must be non-neg
%     m = quadprog(C'*C, C'*f, [], [], Aeq, beq, zeros(numel(nu),1));
